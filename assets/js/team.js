(() => {
  ...
  async function render() {
    ...
    summary.innerHTML = `<span><strong>${teachers.length}</strong> team members</span><span><strong>${active}</strong> active</span><span><strong>${pending}</strong> invitations pending</span>`;
    if (!visible.length) return void (list.innerHTML = `<div class="portal-state">No team members match these filters.</div>`);
